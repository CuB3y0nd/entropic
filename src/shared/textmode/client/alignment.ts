const alignedSelector = ".screen > .home-page, .screen > .home-shell, .screen > .textmode-wrap";
...
export function installTextmodeAlignment(mobileFitBreakpoint: number): void {
  ...
  const align = () => {
    ...
    offsets.forEach(({ element, x, y }) => {
      element.style.setProperty("--pixel-align-x", `${x}px`);
      element.style.setProperty("--pixel-align-y", `${y}px`);
    });
  };
