...
function parseEmail(value: string) {
  ...
  if (!localPart || /[\s<>"\\]/u.test(localPart) || domain.length > 253 || !domainPattern.test(domain)) {
    throw new Error("wkd.email must be an unquoted email address with a valid public DNS domain.");
  }
...
export async function createWkdResources(options: WkdOptions): Promise<WkdResource[]> {
  ...
  if (!email || !publicKeyPath) {
    throw new Error("Set both wkd.email and wkd.publicKeyPath to enable WKD, or leave both unset.");
  }

  const mailbox = parseEmail(email);
  const file = await stat(publicKeyPath);
  if (!file.isFile() || file.size > 1024 * 1024) {
    throw new Error("wkd.publicKeyPath must reference a regular public key file no larger than 1 MiB.");
  }
  ...
  const publicKeys = keys.map((key) => {
    ...
    if (!matches) throw new Error("Every WKD public key must contain a UID matching wkd.email.");
